...
export class EditComponent {
  ...
  constructor(private sessionservice: SessionService, private adminservice: AdminService,  private route: ActivatedRoute, private router: Router, private fb: FormBuilder) {
  ...
  ngOnInit() {
    this.adminservice.getPlan(this.planId).subscribe(
      (retrievedPlan: Plan) => {
        ...
        // Loop through featureOptions and set form controls based on existing data
        this.featureOptions.forEach((feature, i) => {
          const featureName = `feature${i}`;
          if (this.plan.features.includes(feature)) {
            this.editForm.get(featureName)?.setValue(true);
          }
        });
      },
      (error) => {
